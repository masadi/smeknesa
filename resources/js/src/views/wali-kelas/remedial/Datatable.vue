    ...
    <b-overlay :show="loading" rounded opacity="0.6" size="lg" spinner-variant="warning">
      <b-table bordered striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
        ...
        <template v-slot:cell(mapel)="row">
          {{getMapel(row.item.nilai).join(', ')}}
        </template>
        <template v-slot:cell(remedial)="row">
          {{getTp(row.item.nilai)}}
        </template>
        <template v-slot:cell(actions)="row">
          <b-button size="sm" variant="primary" @click="aksi(row.item.peserta_didik_id, 'download')">Download</b-button>
        </template>
      </b-table>
    </b-overlay>
    <b-row class="mt-2">
