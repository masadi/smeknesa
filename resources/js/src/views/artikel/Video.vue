...
import AddModal from './../components/modal/artikel/AddVideo.vue'
...
export default {
  ...
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-video');
    },
    handleReload(){
      this.loadPostsData()
    },
    loadPostsData() {
      ...
      this.$http.get('/referensi/get-video', {
        params: {
    ...
    handleAksi(val){
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`add-video`, val.item);
      }
      console.log(val);
      //console.log(val);
