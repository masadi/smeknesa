...
import AddModal from './../components/modal/artikel/AddSlider.vue'
...
export default {
  ...
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-slider');
    },
    handleReload(){
      this.loadPostsData()
    },
    loadPostsData() {
      ...
      this.$http.get('/referensi/get-slider', {
        params: {
    ...
    handleAksi(val){
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`add-slider`, val.item);
      }
      console.log(val);
      //console.log(val);
