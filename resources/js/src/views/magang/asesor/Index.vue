...
import AddModal from './../../components/modal/magang/asesor/AddModal.vue'
import EditModal from './../../components/modal/magang/asesor/EditModal.vue'
...
export default {
  ...
  created() {
    eventBus.$on('add-asesor', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-asesor');
    },
    handleReload(){
      this.loadPostsData()
    },
      ...
      if(val.aksi === 'hapus'){
        this.$swal({
          ...
          if (result.value) {
            ...
            this.$http.delete(`/magang/asesor/destroy/${val.item.asesor_id}`).then(response => {
        ...
        eventBus.$emit(`open-modal-${val.aksi}-asesor`, val.item);
        this.loading = false
      }
    },
  },
