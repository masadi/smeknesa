...
import AddModal from './../../components/modal/magang/dudi/AddModal.vue'
import EditModal from './../../components/modal/magang/dudi/EditModal.vue'
...
export default {
  ...
  created() {
    eventBus.$on('add-dudi', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-dudi');
    },
    handleReload(){
      this.loadPostsData()
    },
    ...
    handleAksi(val){
      ...
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`open-modal-${val.aksi}-dudi`, val.item);
        this.loading = false
      }
    },
  },
