...
import AddModal from './../../components/modal/magang/absensi/AddModal.vue'
...
export default {
  ...
  created() {
    eventBus.$on('add-absensi-pkl', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-absensi-pkl');
    },
    handleReload(){
      this.loadPostsData()
    },
    ...
    handleAksi(val){
      ...
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$http.post('/magang/hapus-absensi', {
