<template>
  <b-modal v-model="editModalShow" title="Edit Data Pembimbing" size="xl" @hidden="hideModal" @ok="handleOk">
...
export default {
  ...
  created() {
    ...
    eventBus.$on('open-modal-edit-pembimbing', this.handleEvent);
  },
  methods: {
    toggleAll(checked) {
      this.selected = []
    ...
    handleSubmit(){
      ...
      this.$http.post('/magang/update-data', this.form).then(response => {
