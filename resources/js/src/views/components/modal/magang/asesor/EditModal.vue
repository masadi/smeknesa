<template>
  <b-modal v-model="editModalShow" title="Edit Data Asesor" size="lg" @hidden="hideModal" @ok="handleOk">
...
export default {
  ...
  created() {
    ...
    eventBus.$on('open-modal-edit-asesor', this.handleEvent);
  },
  methods: {
    handleEvent(data){
      this.asesor_id = data.asesor_id
      this.$http.get(`/magang/asesor/show/${data.asesor_id}`).then(response => {
        let getData = response.data
        this.form.nama = getData.asesor.nama
        this.form.dudi_id = getData.asesor.dudi_id
        this.data_dudi = getData.dudi
    ...
    handleSubmit(){
      ...
      this.$http.post(`/magang/asesor/update/${this.asesor_id}`, this.form).then(response => {
