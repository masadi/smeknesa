<template>
  <b-modal v-model="editModalShow" title="Edit Data Rencana Penilaian UKK" @hidden="hideModal" @ok="handleOk" size="xl">
    <b-overlay :show="loading_form" rounded opacity="0.6" size="lg" spinner-variant="danger">
      <b-form ref="form" @submit.stop.prevent="handleSubmit">
        <b-row>
          ...
          <b-col cols="12">
            <b-form-group label="Penguji Eksternal" label-for="asesor_id" label-cols-md="3" :invalid-feedback="feedback.asesor_id" :state="state.asesor_id">
              <b-overlay :show="loading_guru" opacity="0.6" size="md" spinner-variant="secondary">
                <v-select id="asesor_id" v-model="form.asesor_id" :reduce="nama => nama.asesor_id" label="nama" :options="data_asesor" placeholder="== Pilih Penguji Eksternal ==" :state="state.asesor_id">
                  <template #no-options="{ search, searching, loading }">
                    Tidak ada data untuk ditampilkan
                  </template>
                </v-select>
...
export default {
  ...
  created() {
    ...
    eventBus.$on('open-modal-edit-rencana-ukk', this.handleEvent);
  },
  methods: {
    handleEvent(data){
      ...
      this.$http.post('/ukk/rencana/show', {
        ...
        this.form.internal = getData.rencana_ukk.internal
        this.form.asesor_id = getData.rencana_ukk.asesor_id
        this.form.tanggal_sertifikat = getData.rencana_ukk.tanggal_sertifikat
        this.form.paket_ukk_id = getData.rencana_ukk.paket_ukk_id
        if(getData.rombongan_belajar){
          this.form.tingkat = getData.rombongan_belajar.tingkat
          this.changeTingkat(this.form.tingkat)
          this.form.rombongan_belajar_id = getData.rombongan_belajar.rombongan_belajar_id
    ...
    handleSubmit(){
      ...
      this.$http.post('/ukk/rencana/update', this.form).then(response => {
