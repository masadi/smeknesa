...
import AddModal from './../../components/modal/referensi/guru-bk/AddModal.vue'
import EditModal from './../../components/modal/referensi/guru-bk/EditModal.vue'
...
export default {
  ...
  created() {
    eventBus.$on('add-bk', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-bk');
    },
    handleReload(){
      this.loadPostsData()
    },
    ...
    handleAksi(val){
      ...
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`open-modal-${val.aksi}-bk`, val.item);
      }
    },
  },
}
