...
import AddModal from './../../components/modal/referensi/bahan-ajar/AddModal.vue'
...
export default {
  ...
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-bahan-ajar');
    },
    handleReload(){
      this.loadPostsData()
    },
    loadPostsData() {
      ...
      this.$http.get('/referensi/bahan-ajar', {
        params: {
    ...
    handleAksi(val){
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`add-bahan-ajar`, val.item);
      }
      console.log(val);
      //console.log(val);
