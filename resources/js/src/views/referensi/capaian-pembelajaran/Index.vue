...
import AddModal from './../../components/modal/referensi/capaian-pembelajaran/AddModal.vue'
import EditModal from './../../components/modal/referensi/capaian-pembelajaran/EditModal.vue'
...
export default {
  ...
  created() {
    eventBus.$on('add-ref-cp', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('open-modal-add-ref-cp');
    },
    handleReload(){
      this.loadPostsData()
    },
    ...
    handleAksi(val){
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`open-modal-${val.aksi}-ref-cp`, val.item);
      }
      console.log(val);
      //console.log(val);
