...
import AddModal from './../../components/modal/referensi/kelompok-mapel/AddModal.vue'
import EditModal from './../../components/modal/referensi/kelompok-mapel/EditModal.vue'
...
export default {
  ...
  data() {
    return {
      ...
      search: '',
      sortBy: 'kurikulum', //DEFAULT SORTNYA ADALAH CREATED_AT
  ...
  created() {
    ...
    eventBus.$on('add-kelompok', this.handleEvent);
    this.loadPostsData()
  },
  methods: {
    handleEvent(){
      eventBus.$emit('loading', true)
      eventBus.$emit('open-modal-add-kelompok');
    },
    handleReload(){
      this.loadPostsData()
    },
    ...
    handleAksi(val){
      if(val.aksi === 'hapus'){
        ...
        eventBus.$emit(`open-modal-${val.aksi}-kelompok`, val.item);
      }
      console.log(val);
      //console.log(val);
